...
//--------------------------------------------query-----------------------------------------
if ($date != "") {
    $title = "รายงานข้อมูลสินค้าวันที่ ". DateThai($date);
...
WHERE tbl_product.created_date ='$date'
GROUP BY tbl_product.pro_id";
} else if ($month != 0 && $year != 0) {
    $title = "รายงานข้อมูลสินค้า เดือน " . month($month) . ' ปี ' . year($year);
...
LEFT JOIN tbl_group_product ON tbl_group_product.gr_id=tbl_product.group_id
LEFT JOIN tbl_type_product ON tbl_type_product.type_pro_id=tbl_group_product.type_pro_id
WHERE MONTH(tbl_product.created_date)='$month' AND  YEAR(tbl_product.created_date) = '$year'
GROUP BY tbl_product.pro_id";
} else if ($year != 0) {
    $title = "รายงานข้อมูลสินค้าประจำปี ". year($year);
...
LEFT JOIN tbl_group_product ON tbl_group_product.gr_id=tbl_product.group_id
LEFT JOIN tbl_type_product ON tbl_type_product.type_pro_id=tbl_group_product.type_pro_id
WHERE  YEAR(tbl_product.created_date) = '$year'
GROUP BY tbl_product.pro_id";
