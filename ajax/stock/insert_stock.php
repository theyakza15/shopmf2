...
for ($count1 = 0; $count1 < count($l1); $count1++) {
        ...
        $query_am =   "SELECT amount FROM tb_color_detail WHERE id_color_det='$mat_id'";

        $result2 = mysqli_query($conn,$query_am);
        $row = mysqli_fetch_assoc($result2);
        $amount_from_db = $row['amount'];

        $total = $amount+$amount_from_db;
        $sql_update = "UPDATE tb_color_detail SET amount='$total' WHERE id_color_det='$mat_id'";
