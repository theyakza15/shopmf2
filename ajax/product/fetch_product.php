...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        $subdata[] = $row['price'];
        $subdata[] = '<img src="images/' . $row['pic'] . '" width="100" height="100"/>';
        $subdata[] = $row['deal_name'];
        $subdata[] = $com_status;
        $subdata[] = '<a href="#edit_pro' . $row['pro_id'] . '" data-toggle="modal" >
        <button type="button" class="btn btn-warning btn-sm" id="btn_edit_product" data="' . $row['pro_id'] . '"
            data-toggle="tooltip"  title="แก้ไขข้อมูล"">
            <i class="fas fa-edit" style="color:white"></i></button>
    </a>' . '
        <button type="button" class="'.$color.'" id="btn_re" data="' . $row['pro_id'] . '"data-status="' . $row['status'] . '" data-name="' . $row['pro_name'] . '"
